...
class _CodeFieldRender extends RenderBox implements MouseTrackerAnnotation {
  ...
  // [dbmaster fork] 仅供 CodeAutocomplete 触发使用的夹紧版位置查询。
  //
  // 上游 bug（0.10.0）：编辑后异步高亮 span 尚未返回时，显示段落仍是旧
  // 文本（_CodeParagraphProvider 按 TextSpan 实例缓存），新光标偏移超出
  // 旧段落长度 → getOffset null → _updateAutoCompleteState 把刚弹出的补
  // 全关掉（第二次起每次击键都误关）。此处把偏移夹紧到段落末尾，位置
  // 略滞后一帧属可接受（弹窗 LayerLink 锚定，段落刷新后自然对齐）。
  Offset? calculateTextPositionScreenOffsetForAutocomplete(CodeLinePosition position, bool rightBottom) {
    final CodeLineRenderParagraph? paragraph = findDisplayParagraphByLineIndex(position.index);
    if (paragraph == null) {
      return null;
    }
    final int clampedOffset = position.offset > paragraph.length
        ? paragraph.length
        : position.offset;
    final Offset? offset = paragraph.getOffset(TextPosition(
