...
class _CodeEditableState extends State<_CodeEditable> with AutomaticKeepAliveClientMixin<_CodeEditable>, SingleTickerProviderStateMixin {
  ...
  void _updateAutoCompleteState(bool isCodeLineChanged) {
    ...
    // [dbmaster fork] 夹紧版位置查询：编辑后异步高亮 span 未返回时显示段落
    // 仍是旧文本，新光标偏移越界会使上游查询返回 null 而把刚弹出的补全
    // 误关（第二次起每次击键都关）。见 _code_field.dart 同名方法注释。
    final Offset? position = render.calculateTextPositionScreenOffsetForAutocomplete(widget.controller.selection.extent, true);
    if (position == null) {
      autocompleteState.dismiss();
      return;
    }
