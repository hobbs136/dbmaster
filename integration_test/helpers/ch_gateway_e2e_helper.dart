// T29 第三批 · CH E2E 共享前置 —— embedded server 会话注入。
//
...
//
// 与 test/helpers/ch_gateway_live_helper.dart 的区别：那边面向
...
/// connected。
Future<bool> ensureEmbeddedServerForChE2E() async {
  ...
  if (!embedded.isRunning) {
    ...
    if (result != EmbeddedStartResult.started) {
      // ignore: avoid_print
      print(
        'CH_E2E_SKIP: embedded start result=$result '
  ...
  if (!connected) {
    // ignore: avoid_print
    print('CH_E2E_SKIP: embedded handshake did not reach connected');
  }
  return connected;
}
