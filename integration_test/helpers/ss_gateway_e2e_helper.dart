// T28 · SQL Server E2E 共享前置 —— embedded server 会话注入。
//
// SS 网关壳（sqlserver_gateway_adapter）的执行通道是 T27 网关 API，硬依赖
...
// SQLSERVER_E2E_SKIP 打印并整体跳过（FR-011 无假绿纪律）。
...
/// connected。
Future<bool> ensureEmbeddedServerForSsE2E() async {
  ...
  if (!embedded.isRunning) {
    ...
    if (result != EmbeddedStartResult.started) {
      // ignore: avoid_print
      print(
        'SQLSERVER_E2E_SKIP: embedded start result=$result '
  ...
  if (!connected) {
    // ignore: avoid_print
    print('SQLSERVER_E2E_SKIP: embedded handshake did not reach connected');
  }
  return connected;
}
