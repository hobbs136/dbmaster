// T29 第二批 · PG E2E 共享前置 —— embedded server 会话注入。
//
...
//
// 与 test/helpers/pg_gateway_live_helper.dart 的区别：那边面向
// `flutter test` VM 环境（无插件通道，Process.start 直启 + 手工握手解析）；
// 本文件面向 integration_test（插件可用，直接走 EmbeddedServerService）。
//
// 二进制不可得（未构建/未部署）时返回 false——各 E2E 组以可 grep 的
// PG_E2E_SKIP 打印并整体跳过（无假绿纪律）。
...
/// connected。
Future<bool> ensureEmbeddedServerForPgE2E() async {
  ...
  if (!embedded.isRunning) {
    ...
    if (result != EmbeddedStartResult.started) {
      // ignore: avoid_print
      print(
        'PG_E2E_SKIP: embedded start result=$result '
  ...
  if (!connected) {
    // ignore: avoid_print
    print('PG_E2E_SKIP: embedded handshake did not reach connected');
  }
  return connected;
}
