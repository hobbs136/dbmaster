// T29 非 SQL 批次（B2）· MongoDB E2E 共享前置 —— embedded server 会话注入。
//
// Mongo 网关壳（mongodb_gateway_adapter）的执行通道是 kind:"mongo" 的
// runCommand 网关 API（ADR-0006），硬依赖 dbmaster server 会话（D6：
// embedded exe 随包）。E2E 前置流程与 pg_gateway_e2e_helper 同构：
...
/// connected。
Future<bool> ensureEmbeddedServerForMongoE2E() async {
  ...
  if (!embedded.isRunning) {
    ...
    if (result != EmbeddedStartResult.started) {
      // ignore: avoid_print
      print(
        'MONGO_E2E_SKIP: embedded start result=$result '
  ...
  if (!connected) {
    // ignore: avoid_print
    print('MONGO_E2E_SKIP: embedded handshake did not reach connected');
  }
  return connected;
}
