// T29 第二批 · PG 网关 live 测试共享前置 —— embedded server 会话注入。
//
// PG 网关壳（postgresql_gateway_adapter）的执行通道是 T27 网关 API，硬依赖
// dbmaster server 会话（D6：embedded exe 随包）。与 integration_test 的
// mysql_gateway_e2e_helper 同构，但面向 `flutter test` VM 环境：
...
// grep 的 PG_LIVE_SKIP 打印并跳过（无假绿纪律；恢复条件：构建 server
...
Future<bool> ensureEmbeddedServerForPgLive() async {
  ...
  final dataDir = await Directory.systemTemp.createTemp('pg_live_gw_');
...
Future<void> stopEmbeddedServerForPgLive() async {
  final proc = _pgLiveServer;
  _pgLiveServer = null;
  if (proc != null) {
    proc.kill(ProcessSignal.sigkill);
  }
...
String? _besideExe() {
  ...
  final beside =
      File('${File(Platform.resolvedExecutable).parent.path}${Platform.pathSeparator}$name');
